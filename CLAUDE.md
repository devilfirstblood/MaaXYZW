...
- `daily_all.js` — 「每日任务一键全完成」，每天定点。入口 `全日常_启动` + override（各 `xxx_完成`→复位节点）。
- `account_login.js` — 独立的「微信登录」检测与提醒。
- `app_screencap_report.js` — 启动指定 App 截图上报企微群（`com.willh.wz` 是「咸鱼之王微信扫码登录工具」，`apk/GameWxQRlogin-xyzw.apk`，调出专属授权二维码）。
