...
- `daily_chain.js` — 咸鱼之王「一条龙」（挂机+盐罐），定时循环。入口 `咸鱼_启动游戏` + override。 一轮内最多轮转 3 个账号(做完一个号→切号→再做，账号2+从 挂机_入口 热接入)；第 5 参为每账号超时分钟数；收尾不关游戏。
- `daily_all.js` — 「每日任务一键全完成」，每天定点。入口 `全日常_启动` + override：前段各活动 `xxx_完成`→复位节点串成主干，尾段再接 领奖→收邮件(`邮件_启动`)→领福利(`充值_启动`)→咸王→钓鱼→俱乐部→珍宝阁。每做完一个账号还按当天星期额外跑周期性活动：周一答题、周二/四/六扭蛋、周一/三梦境(互不阻塞主链，失败只记日志不重试)。 第 5 参为每账号超时分钟数；一轮内最多轮转 3 个账号。
...
- **已接入 `daily_all.js`**：每轮内做完一个号 → 读标题关卡数去重 → 挑未做过的切换位切号 → 再跑，最多 3 个（挑位纯逻辑在 `scripts/lib/account_rotation.js`；无其它账号时只做 1 个，向后兼容）。
